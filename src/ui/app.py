...
# Make src/ imports work when launched from any directory
...
st.markdown(
    ...
    h1 { font-weight: 700; letter-spacing: -0.03em; margin-bottom: -0.2rem; }
    .header-sub { font-size: 0.95rem; color: #6b7280; margin-bottom: 1.5rem; }
    .card {
        background: #ffffff;
        border-radius: 18px;
...
# ---------------------------------------------------------
st.markdown(
    '<div class="section-title">Étape 1 · Charger vos documents</div>',
    unsafe_allow_html=True,
)
with st.form("setup"):
...
if submitted:
    if not cv_file or not job_url:
        ...
        with st.spinner("Analysing CV + job…"):
            ...
            # Persist to disk so the LiveKit worker can load them
            export_cv(cv_data)
            export_job(job_data)
...
# ---------------------------------------------------------
# If data exists → show preview + simulation
# ---------------------------------------------------------
if "cv" in st.session_state and "job" in st.session_state:
    ...
    # ---------------- Preview ----------------
    st.markdown(
        '<div class="section-title">Étape 2 · Aperçu des données</div>',
        unsafe_allow_html=True,
    )
    cA, cB = st.columns(2)
    ...
    if "history" in st.session_state:
        st.markdown(
            '<div class="section-title">Étape 4 · Résumé</div>',
            unsafe_allow_html=True,
        )
